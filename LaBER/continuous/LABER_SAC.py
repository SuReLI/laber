...
class LABER_SAC(object):
    ...
    def train(self, replay_buffer, batch_size=256, m_factor=4):
        self.total_it += 1
        ...
        with torch.no_grad():
            ...
            # take the mean of both critics
            Q_target_next = torch.min(Q_target1_next, Q_target2_next)
            # Compute Q targets for current states (y_i)
            Q_target_next_large = reward + not_done * self.discount * (Q_target_next - self.alpha * log_pis_next)

            # Select uniformly a subset of transitions for the actor
            ...
            ## Select a subset of transitions for the second critic
            # Compute surrogate distribution
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            # Compute alpha loss
            actions_pred, log_pis = self.actor.evaluate(states_selected_for_actor)
            ...
            self.alpha = alpha
            # Compute actor loss
            actor_loss = (alpha * log_pis.squeeze(0) - self.critic1(states_selected_for_actor, actions_pred.squeeze(0))).mean()

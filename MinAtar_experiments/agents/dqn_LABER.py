...
#
################################################################################################################
def train(mfactor, sample, policy_net, target_net, optimizer):
    ...
    with torch.no_grad(): # The forward pass on the large mini-batch is performed with no_grad
    ...
    if len(none_terminal_next_states) != 0:
        with torch.no_grad():
            Q_s_prime_a_prime[none_terminal_next_state_index] = target_net(none_terminal_next_states).max(1)[0].unsqueeze(1)

    # Compute the target
    target = rewards + GAMMA * Q_s_prime_a_prime

    # Compute priorities on the large batch. Recall the Huber loss is used, so the priorities are
    # min(|td errors|, 1)
    ...
    # Select the mini-batch according to priorities
    indices = numpy.random.choice(numpy.arange(int(mfactor*BATCH_SIZE)), BATCH_SIZE, p=probs)
    td_errors_for_selected_indices = td_errors[indices]
    states_selected = states[indices]
    actions_selected = actions[indices]
    ...
    #LaBER-SUM (change sum in max to have LaBER-MAX)
    #loss_weights = loss_weights / sum(loss_weights) 
    #LaBER
    loss_weights = 1.0 / td_errors_for_selected_indices
    mean_td_errors = td_errors.mean()
    loss_weights = loss_weights * mean_td_errors
        ...
        while(not is_terminated) and t < NUM_FRAMES:
            ...
            if replay_off:
                ...
                if t > REPLAY_START_SIZE and len(r_buffer.buffer) >= int(mfactor*BATCH_SIZE):
                    # Sample a batch
                    sample = r_buffer.sample(int(mfactor*BATCH_SIZE))

            # Train every n number of frames defined by TRAINING_FREQ
            if t % TRAINING_FREQ == 0 and sample is not None:
                if target_off:
                    train(mfactor, sample, policy_net, policy_net, optimizer)
                else:
                    policy_net_update_counter += 1
                    train(mfactor, sample, policy_net, target_net, optimizer)

...
#   -m, --mfactor: size of the large batch                                                                     #
#                                                                                                              #
...
class replay_buffer:
    def __init__(self, buffer_size):
        ...
        self.location = 0
        self.buffer = []
        self.priorities = numpy.zeros((buffer_size,), dtype=numpy.float32)
    ...
    def add(self, *args):
        ...
        self.priorities[self.location] = max_prio
    ...
    def sample(self, batch_size):
        ...
        indices = numpy.random.choice(N, batch_size, p=P) 
...
#
################################################################################################################
def train(mfactor, sample, weights, indices, policy_net, target_net, optimizer):
    # Batch is a list of namedtuple's, the following operation returns samples grouped by keys
    # Samples are selected according to the priorities associated to each transition of the replay buffer
    ...
    # Same code as LABER
    td_errors = abs(target - Q_s_a)
    td_errors = td_errors.squeeze().numpy()
    td_errors = numpy.minimum(td_errors, numpy.ones(int(mfactor*BATCH_SIZE)))
    probs = td_errors/td_errors.sum()
    probs = probs/numpy.sum(probs)

    batch_indices = numpy.array(numpy.random.choice(numpy.arange(mfactor*BATCH_SIZE), BATCH_SIZE, p=probs), dtype=numpy.int32)
    td_errors_for_selected_indices = td_errors[batch_indices]
    states_selected = states[batch_indices]
    actions_selected = actions[batch_indices]

    new_target = target[batch_indices]
    autograd_hacks.add_hooks(policy_net)
    new_Q_s_a = policy_net(states_selected).gather(1, actions_selected)
    
    loss_weights = 1.0 / td_errors_for_selected_indices
    loss_weights = loss_weights * td_errors.mean()
    loss_weights = torch.from_numpy(loss_weights).unsqueeze(1)

    # Huber loss
    loss = f.smooth_l1_loss(new_target, new_Q_s_a, reduction='none')
    loss = loss * loss_weights
    loss = loss.mean()


    # Zero gradients, backprop, compute per-sample gradient norms, update the weights of policy_net    
    optimizer.zero_grad()
    loss.backward(retain_graph=True)
    autograd_hacks.compute_grad1(policy_net)    
    autograd_hacks.disable_hooks()
    autograd_hacks.clear_backprops(policy_net)
    autograd_hacks.remove_hooks(policy_net)

    grad_norms = numpy.zeros(BATCH_SIZE)
    for layer in policy_net.modules():
        if not autograd_hacks.is_supported(layer):
            continue
        for param in layer.parameters():
            for i in range(BATCH_SIZE):
                grad_norms[i] += numpy.linalg.norm(param.grad1[i].numpy())**2
    grad_norms = numpy.sqrt(grad_norms) / loss_weights.squeeze(1).numpy() 

    optimizer.step()
    
    return indices, grad_norms
...
#
#################################################################################################################
def dqn(n, env, replay_off, target_off, output_file_name, store_intermediate_result=False, load_path=None, step_size=STEP_SIZE, mfactor=1.0):
    ...
    while t < NUM_FRAMES:
        ...
        while(not is_terminated) and t < NUM_FRAMES:
            ...
            if replay_off:
                ...
                if t > REPLAY_START_SIZE and len(r_buffer.buffer) >= int(mfactor*BATCH_SIZE):
                    # Sample a batch
                    sample, weights, indices = r_buffer.sample(int(mfactor*BATCH_SIZE))

            # Train every n number of frames defined by TRAINING_FREQ
            if t % TRAINING_FREQ == 0 and sample is not None:
                if target_off:
                    indices, grad_norms = train(mfactor, sample, weights, indices, policy_net, policy_net, optimizer)
                    r_buffer.update_priorities(indices, grad_norms)
                else:
                    policy_net_update_counter += 1
                    indices, grad_norms = train(mfactor, sample, weights, indices, policy_net, target_net, optimizer)
                    r_buffer.update_priorities(indices, grad_norms)
        ...
        if store_intermediate_result and e % 250 == 0:
            torch.save({
                        'episode': e,
                        'frame': t,
...
def main():
    ...
    parser.add_argument("--mfactor", "-m", type=float, default=1.0)
    ...
    dqn(args.game, env, args.replayoff, args.targetoff, file_name, args.save, load_file_path, args.alpha, args.mfactor)

...
#
################################################################################################################
def train(sample, weights, indices, policy_net, target_net, optimizer):
    ...
    loss_weights = torch.from_numpy(weights).unsqueeze(1)

    # Huber loss
    loss = f.smooth_l1_loss(target, Q_s_a, reduction='none').cpu()
    ...
    grad_norms = numpy.sqrt(grad_norms) / weights
    #grad_norms = (grad_norms + 1e-10)**0.6
...
#
#################################################################################################################
def dqn(n, env, replay_off, target_off, output_file_name, store_intermediate_result=False, load_path=None, step_size=STEP_SIZE):
        ...
        while(not is_terminated) and t < NUM_FRAMES:
            ...
            if replay_off:
                ...
                if t > REPLAY_START_SIZE and len(r_buffer.buffer) >= BATCH_SIZE:
                    # Sample a batch
                    sample, weights, indices = r_buffer.sample(BATCH_SIZE)

            # Train every n number of frames defined by TRAINING_FREQ
            if t % TRAINING_FREQ == 0 and sample is not None:
                if target_off:
                    indices, grad_norms = train(sample, weights, indices, policy_net, policy_net, optimizer)
                    r_buffer.update_priorities(indices, grad_norms)
                else:
                    policy_net_update_counter += 1
                    indices, grad_norms = train(sample, weights, indices, policy_net, target_net, optimizer)
                    r_buffer.update_priorities(indices, grad_norms)
...
def main():
    ...
    dqn(args.game, env, args.replayoff, args.targetoff, file_name, args.save, load_file_path, args.alpha)

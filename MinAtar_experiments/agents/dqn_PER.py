...
def train(sample, weights, indices, policy_net, target_net, optimizer):
    ...
    td_errors = abs(Q_s_a - target).detach().cpu().numpy()
    td_errors = td_errors + 1e-10
    ...
    while t < NUM_FRAMES:
        ...
        while(not is_terminated) and t < NUM_FRAMES:
            ...
            if t % TRAINING_FREQ == 0 and sample is not None:
                if target_off:
                    indices, td_errors = train(sample, weights, indices, policy_net, policy_net, optimizer)
                    r_buffer.update_priorities(indices, td_errors)
                else:
                    policy_net_update_counter += 1
                    indices, td_errors = train(sample, weights, indices, policy_net, target_net, optimizer)
                    r_buffer.update_priorities(indices, td_errors)

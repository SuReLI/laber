...
def train(mfactor, sample, weights, indices, policy_net, target_net, optimizer):
    ...
    # Same code as LABER
    td_errors1 = abs(target - Q_s_a)
    td_errors1 = td_errors1.squeeze().numpy()
    td_errors = numpy.minimum(td_errors1, numpy.ones(int(mfactor*BATCH_SIZE)))
    ...
    while t < NUM_FRAMES:
        ...
        while(not is_terminated) and t < NUM_FRAMES:
            ...
            if t % TRAINING_FREQ == 0 and sample is not None:
                if target_off:
                    indices, td_errors = train(mfactor, sample, weights, indices, policy_net, policy_net, optimizer)
                    r_buffer.update_priorities(indices, td_errors)
                else:
                    policy_net_update_counter += 1
                    indices, td_errors = train(mfactor, sample, weights, indices, policy_net, target_net, optimizer)
                    r_buffer.update_priorities(indices, td_errors)
